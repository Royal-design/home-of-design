  ...
  return (
    <article className="group card-surface card-surface-hover flex h-full flex-col p-3 sm:p-4">
      <div className="card-media card-media-hover relative aspect-[4/5] overflow-hidden">
        ...
        <div className="absolute inset-x-4 bottom-4 flex translate-y-2 items-center justify-between gap-2 opacity-100 transition-all duration-500 ease-expo-out group-hover:translate-y-0 group-hover:opacity-100 max-sm:translate-y-0 max-sm:opacity-100">
          <button
            type="button"
            onClick={(e) => {
              e.preventDefault();
            ...
            className={cn(
              "flex cursor-pointer items-center gap-2 bg-ink px-2 py-2 font-mono text-[0.6rem] uppercase tracking-[0.18em] text-paper transition-colors duration-300 hover:bg-bronze md:px-4 md:py-3",
            )}
          >
            <Plus size={13} />
            Add
          ...
          <button
            ...
            className={cn(
              "flex h-7 w-7 cursor-pointer items-center justify-center border backdrop-blur-sm transition-all duration-300 md:h-11 md:w-11",
              isFavourite
      ...
      <div className="mt-4 flex flex-1 flex-col px-1 pb-1 sm:px-0.5">
        ...
        <Link
          to={`/products/${product.id}`}
          className="mt-1 line-clamp-2 text-sm leading-5 text-ink transition-colors hover:text-bronze md:text-[15px]"
        >
          {product.name}
        </Link>

        <div className="mt-3 flex flex-1 flex-col md:flex-row md:items-center md:gap-4 ">
          {discount > 0 && (
          ...
          <span className="min-w-0 break-words font-mono text-base font-semibold leading-tight text-ink">
